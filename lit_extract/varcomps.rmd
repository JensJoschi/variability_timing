...
for (dataset in (num[1]:num[2])){
  ...
  currpop <- unique(data$popid)[dataset]
  res<-read.table(file = paste("res_", currpop, ".txt", sep = "" ))
  if (!is.na(res[1,1])){ #should leave 22 empy lines
   for ( i in 1:chains){
...
par(mfrow = c(1,3),mar = c(4,2,1,0))
plot_cdl(xlab = "Mean Timing", xlim = c(8,24),ylim = c(1,52), num = c(l[1]+1,l[2]),scale=  0.9)
plot_cdl("Mean Timing", c(8,24), c(1,52), c(l[2]+1,l[3]), 0.9)
plot_cdl( "Mean Timing", c(8,24), c(1,52), c(l[3]+1,l[4]), 0.9)

plot_cdl( "Mean Timing", c(8,24), c(1,52), c(l[4]+1,l[5]), 0.9)
plot_cdl( "Mean Timing", c(8,24), c(1,52), c(l[5]+1,l[6]), 0.9)
plot_cdl( "Mean Timing", c(8,24), c(1,52), c(l[6]+1,l[7]), 0.9)

plot_cdl( "Mean Timing", c(8,24), c(1,52), c(l[7]+1,l[8]), 0.9)
plot_cdl( "Mean Timing", c(8,24), c(1,52), c(l[8]+1,l[9]), 0.9)
plot_cdl( "Mean Timing", c(8,24), c(1,52), c(l[9]+1,447), 0.9)
dev.off()
```
...
for (dataset in 1:num){
  ...
  estimates_cdl[dataset,]<-quantile(res[,21], c(0.025,0.5,0.975))
}
}
