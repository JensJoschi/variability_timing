...
metaf<-rma.mv(yi = e ~ degN, V=1/npoints, random = ~1|order/genus/spec/ID/popid,data = slopes)
...
lines(x=range(slopes$degN),y=degN_red2$coefficients$fixed[1] + degN_red2$coefficients$fixed[2] * range(slopes$degN),lwd=2)
lines(x=slopes$degN,y=ps$pred,lwd=2,col=2)#a reminder that the prediction interval is based on metafor model
...
for(i in 1:length(unique(slopes$order))){
  ...
  studmod<-rma.mv(yi = e ~ degN,V=1/npoints, data = sub,random = ~1|genus/spec/popid)
