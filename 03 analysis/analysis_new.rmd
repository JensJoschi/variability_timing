...
AIC(degN_red2.ml)-AIC(nullme.ml)#-175.8718
```
...
coffs <-  summary(degN_red2)$tTable
lines(x= range(slopes$degN),y = coffs[1,1]+coffs[2,1]*range(slopes$degN),lwd=3,lty=2)
...
for(i in c(2,6,8)){
  ...
  coffs <-  summary(sinord)$tTable
  lines(x= range(sub$degN),y = coffs[1,1]+coffs[2,1]*range(sub$degN), lwd= 1+round(5* table(slopes$order)[i]/max(table(slopes$order))), col=sub$col[1])
  print(sub$order[1])
  print (paste("slope: ",(coffs[2,1]*5*60), "se: ", (coffs[2,2]*5*60)))
}
points(x=slopes$degN ,y=slopes$e, pch=21, col=1, bg=slopes$col, cex=2*slopes$npoints/10)
dev.off()
#slope estimates: 
#Diptera: slope:  64.2181986214667 se:  2.68069414915251
#Lepidoptera "slope:  69.0967518142614 se:  8.22812962302004"
#mites: "slope:  49.9671137736757 se:  7.39115976448246"
```
