...
profile(rmamod_nested, sigma2=3)#order/g/species #very flat
profile(rmamod_nested, sigma2=4)#order/g/species/study
profile(rmamod_nested, sigma2=5)#order/g/species/study/population
```
...
profile(reduced, sigma2=4)#order/g/study/population
...
profile(reduced2, sigma2=4)#order/g/spec/population
...
#all as ML fits
reduced2.ml<-rma.mv(yi = e ~ degN, V=1/npoints, random = ~1|order/genus/spec/popid,data = slopes, method= "ML")
nullmod.ml<-rma.mv(yi = e ~ 1, V=1/npoints, random = ~1|order/genus/spec/popid, data = slopes, method="ML")
nullme.ml<-lme(e ~ 1, random =~ 1|order/genus/spec, data=slopes, weights=~npoints, method="ML")
lme.ml<-lme(e ~ degN, random =~ 1|order/genus/spec, data=slopes, weights=~npoints, method="ML")
...
The following chunk calculates summary statistics, I², R² and statistical significance of the main effect.
confidence/prediction intervals, sigma estimates and LRT tests and plots are based on metafor, while R² is based on lme. LRT and sigma from lme is very similar to metafor though, and marginal R² from metafor is similar to lme.
...
#estimate +ci
confint(reduced2,fixed=T,random=F)
#0.2020 ( 0.1848 - 0.2156)
#confint(reduced2,fixed=T,random=F)$fixed[2,]*5*60
#=60.06726 (55.45489 - 64.67962)  min
```

_ I²_  
The generalized calculation of I² is described in http://www.metafor-project.org/doku.php/tips:i2_multilevel_multivariate .
one can calculate the overall I² and the individual contribution of each random level:
```{r degN_isq}
W <- diag(slopes$npoints)
X <- model.matrix(rmamod_nested)
P <- W - W %*% X %*% solve(t(X) %*% W %*% X) %*% t(X) %*% W 
100 * sum(reduced2$sigma2) / (sum(reduced2$sigma2) + (reduced2$k-reduced2$p)/sum(diag(P)))
#I² of each level
x<-round(100 * reduced2$sigma2 / (sum(reduced2$sigma2) + (reduced2$k-reduced2$p)/sum(diag(P))),4)
x
#heterogeneity is apparently 98.62%, 35%,48% and 14% due to order/genus/ID
```

I am not sure whether the I² calculation makes sense, because it calculates sample variance relative to overall variance. Does it make sense to compare sample size variation to overall variance? What is maybe more important is the relative contribution of each level:
```{r degN_Icont}
x/sum(x)
# that is the same as:
reduced2$sigma2/sum(reduced2$sigma2)

#the same calculation based on the standard deviations of the random terms from the lme model:
sdevs/sum(sdevs)
```
...
```{r degN_R}
#table of all possible R² calculations
rsq<-data.frame(metafor = rep(NA,2), lme = rep(NA,2))
rownames(rsq) = c("marginal","conditional")

#appraoch advertised by W.viechtbauer, maker of metafor:
(sum(nullmod$sigma2)-sum(reduced2$sigma2))/sum(nullmod$sigma2) #0.54
#possibly wrong, sigma may not be correctly calculated for sample-size based approach 
rsq[1,1]<-summary(lm(reduced2$yi~fitted(reduced2), weights = 1/reduced2$vi))$adj.r.squared #0.73
#the simpler yi~fitted method

rsq[1,2]<-r.squaredGLMM(lme)[1]#marginal R², from library MuMin
#0.77

# simpler method for the marginal R² (the random terms need to be ignored which boils down to lm):
fixedmod<-lm(slopes$e~slopes$degN,weights= slopes$npoints)
summary(lm(slopes$e~fitted(fixedmod)))$adj.r.squared
#0.76

#conditional  
rsq[2,1] <-NA # fitted with blups is not available for rma.mv

rsq[2,2] <- r.squaredGLMM(lme)[2]
#or alternatively 
summary(lm(slopes$e~fitted(lme)))$adj.r.squared#0.90
rsq
```
only the lme R² are the nakagawa approaches.

_significance: LRT tests_  
```{r degN_sig}
anova(lme.ml,nullme.ml) #LRT ratio = 177.8717, p<0.001
anova(reduced2.ml,nullmod.ml) #LRT ratio = 176.0836, p<0.001
```
