...
with the estimate weighted by the inverse of its standard error. One could argue that study can be erased because it is almost entirely covered by species. I will keep it in though, because random effects with nearly 0 explained variance have no effect anyway. The only package (i am aware of) that can do the correct weighting is metafor, and it now also supports nested random terms.
...
forest.default(x=r2$e,sei=r2$real_se,slab=NA,xlim=c(8,20),annotate=T) #same
...
r2$e_se<-1/r2$e_se #that inversion was included in DRC estimation script, need to revert it here
r2<-r2[!is.na(r2$spec),]
r2<-droplevels(r2)

#### a simple (but wrong) rma model (to test metafor)#### 
rmamod<-rma(yi=e,vi=e_se,mods = degN,data=r2,method="REML",weighted=T,test="knha")
summary(rmamod)
confint(rmamod)
ps<-predict.rma(rmamod)

normal<- data.frame(r2$degN,ps$cr.lb,ps$ci.lb)
names(normal)<-c("degN","cr","ci")
normal<-normal[order(normal$degN),]
reversed<-data.frame(r2$degN,ps$cr.ub,ps$ci.ub)
...
plot(x=r2$degN,y=r2$e,pch=21,cex=0.8,col=NA, bg=as.numeric(r2$spec))
...
points(x=r2$degN,y=r2$e,pch=21,cex=0.8,col=NA,bg=as.numeric(r2$spec))

#comparing that to a lme
summary(lme(e~degN,random= ~1|ID,data=r2)) #similar bot not exactly same (due to lack of weighting)
summary(lme(e~degN,random= ~1|ID,data=r2, weights= ~e_se)) #nearly same
...
####now to the actual model with nested random terms#### 
rmamod_nested<-rma.mv(yi = e ~ degN, V = e_se, random = ~1|order/g/s/study, data = r2,test="t")
summary(rmamod_nested) # species has no variance explained

rmamod_rednested<-rma.mv(yi = e ~ degN, V = e_se, random = ~1|order/g/study,data = r2)
summary(rmamod_rednested)
summary(lme(e~degN, random =~ 1|order/g/study,data=r2,weights=~e_se)) #very similar
...
#3. forest plot
fore<-data.frame(ps$pred,ps$ci.lb,ps$ci.ub,r2$order,r2$g,r2$s,r2$study)
fore<-fore[order(fore$r2.order,fore$r2.g,fore$r2.s,fore$r2.study),]

plot(y=1:nrow(fore),x=fore$ps.pred,xlim=c(8,21),pch=22,cex=0.8,main ="Forest plot of CDL estimates",xlab = "Critical day length",yaxt="n",ylab="",bg=fore$r2.study,col=NA)
arrows(x0=fore$ps.pred,y0=1:nrow(fore),x1=fore$ps.ci.ub,length=0,col=1,lwd=1.5)
arrows(x0=fore$ps.pred,y0=1:nrow(fore),x1=fore$ps.ci.lb,length=0,col=1,lwd=1.5)
points(y=1:nrow(fore),x=fore$ps.pred,pch=22,cex=0.5,bg=fore$r2.study,col=NA)
abline(h=cumsum( as.numeric(table(fore$r2.order)))+0.5,lty=2,lwd=0.5)
#need to play with colours so that species of same order have similar colours


#4. pseudo-R^2
pseudo_nested<-rma.mv(yi = e ~ 1, V = e_se, random = ~1|order/g/s/study,data = r2,test="t")
rsq<-(sum(pseudo_nested$sigma2)-sum(rmamod_nested$sigma2))/sum(pseudo_nested$sigma2)
#0.4372

#5 test what lme would produce (should be consistent)
M3<-lme(e~degN ,random=~ 1|order/g/s/study,data=r2, weights= ~e_se) 
summary(M3)#quite similar, though some diff in estimate and s.e. exists
...
plot(x=r2$degN,y=r2$e,pch=21,cex=0.8,col=NA, bg=as.numeric(r2$spec),main = "Delay in photoperiodic response vs latitude", xlab = "Latitude (°N)", ylab = "Critical photoperiod")
polygon(x=combined$degN, y=combined$cr,col="lightgrey",border=NA)
polygon(x=combined$degN, y=combined$ci,col="darkgrey",border=NA)
lines(x=r2$degN,y=ps$pred,lwd=2)
points(x=r2$degN,y=r2$e,pch=21,cex=0.8,col=NA,bg=as.numeric(r2$spec))
axis(1)
...
text(30,20,paste("slope = ",s_est, " hours/5°N\npseudo-R² = ",rsq),cex=0.8)


###clean up from here downwards
