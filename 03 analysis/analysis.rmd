...
with the estimate weighted by the inverse of its standard error. One could argue that study can be erased because it is almost entirely covered by species. I will keep it in though, because random effects with 0 explained variance have no effect anyway. The 2 most frequently functions to do the modelling are lme (package nlme) and lmer(package lme4). lme has the advantage that it also can model autocorrelation structures, so I will first stick to that one.  
