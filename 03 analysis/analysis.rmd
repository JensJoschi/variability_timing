...
---
#List of issues
check whether e_rel in slopes.rmd is se or 1/se
some climate stations exist 2*
...
Mean winter decreases by 2.5 days per ° latitude = 12.5 days per 5°, so an organism needs to react to a day length equal to 12.5 days earlier in the year per 5°N. correct calculation of day length at a given latitude for a given day: https://en.wikipedia.org/wiki/Sunrise_equation
...
```{r,include = FALSE}
hist(climate$alt)
climate$alt[climate$alt==-999.9]<-NA
climate$sq_alt <-sqrt(climate$alt)
...
hist(climate$unpredictability,breaks=100)
sum(!is.na(climate$unpredictability))
sum(is.na(climate$unpredictability))
length(climate$unpredictability[climate$unpredictability>5])-3911
97/16923 #0.5% of all values are >5, 1.1%>4
hist(climate$unpredictability[climate$unpredictability<5],breaks=100)
```
...
Take the 5 closest climate stations in a 5° radius, calculate sd,predictability and mean of these weighted by inverse of euclidian distance
get for each study site the closest climate stations
...
for ( i in 1:nrow(slopes)){
  ...
  sub$diffN<-sub$lat-slopes[i,10]
  sub$diffE<-sub$lon-slopes[i,11]
  sub$diff<-sqrt(sub$diffN^2+sub$diffE^2)
  sub<-arrange(sub,diff)[1:5,]
...
#not according to hypotheses
#plot(slopes$b~slopes$meanwinter,xlab = "mean winter",ylab="slope of PRC")
#plot(slopes$e~slopes$sd_winter,xlab = "standard deviation in winter",ylab = "CDL")
#plot(slopes$e~slopes$unpredictability,xlab = "winter unpredictability",ylab = "CDL")
