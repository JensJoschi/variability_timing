...
It is possible a bad idea to use any climate above 70°N. The highest latitude in the studies was 69.05 °N. The apparent cap at ~25°N is because winter is not defined for warm climates (winter never arrives). The lowest latitude in the studies is at 18.3°N. 
Refined overview:  
...
sum(is.na(climate$unpredictability))
length(climate$unpredictability[climate$unpredictability>5])-25 #values >5 but not NA
159/24227 #values >5 divided by non-Nas = 0.65% of all values are >5
hist(climate$unpredictability[climate$unpredictability<5],breaks=100)

climate$beta<-log(climate$beta)
climate$beta<--1* climate$beta
hist(climate$beta,breaks=100)
```
...
p<-p[!is.na(p$beta),]
#p[p$beta>0.6]<-0.6
plot(p$lat~p$lon,bg=rgb(1,max(p$beta)-p$beta+0.0001,max(p$beta)-p$beta+0.0001,maxColorValue = max(p$beta+0.001)),cex=0.5,pch=22,col=NA)
```
