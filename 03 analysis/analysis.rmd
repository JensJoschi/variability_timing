...
5) calculate a pseudo-R²: (sum(Model1$sigma2) - sum(Model2$sigma2)) / sum(Model1$sigma2) where model 2 is full model, model 1 has no fixed effect
...
forest.default(x=r2$e,sei=r2$real_se,slab=NA,xlim=c(8,20),annotate=T)
...
#a simple (but wrong) rma model (to learn use of metafor)
rmamod<-rma(yi=e,vi=e_se,mods = degN,data=r2,method="REML",weighted=T,test="knha")#slab=study
summary(rmamod)
confint(rmamod)
ps<-predict.rma(rmamod)
plot(x=r2$degN,y=r2$e,pch=21,cex=0.8,col=NA, bg=as.numeric(r2$spec))
lines(x=r2$degN,y=ps$pred,lwd=2)
lines(x=r2$degN,y=ps$ci.lb,col="darkgrey")#convert these into polygons
lines(x=r2$degN,y=ps$ci.ub,col="darkgrey")
lines(x=r2$degN,y=ps$cr.lb,col="lightgrey")
lines(x=r2$degN,y=ps$cr.ub,col="lightgrey")
#points again
...
rmamod_nested<-rma.mv(yi = e ~ degN, V = e_se, random = ~1|order/g/s/study,data = r2)
...
#3. plot prediction + credible interval
ps<-predict.rma(rmamod_nested)
plot(x=r2$degN,y=r2$e,pch=21,bg=r2$study,cex=0.8,col=NA)
lines(x=r2$degN,y=ps$pred,lwd=2)
lines(x=r2$degN,y=ps$ci.lb,col="darkgrey")#convert these into polygons
lines(x=r2$degN,y=ps$ci.ub,col="darkgrey")
lines(x=r2$degN,y=ps$cr.lb,col="lightgrey")
...
#5. pseudo-R^2
pseudo_nested<-rma.mv(yi = e ~ 1, V = e_se, random = ~1|order/g/s/study,data = r2)
(sum(pseudo_nested$sigma2)-sum(rmamod_nested$sigma2))/sum(pseudo_nested$sigma2)
#0.4372


#stuff below is possib#scaling!
