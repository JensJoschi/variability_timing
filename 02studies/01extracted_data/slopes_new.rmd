...
length(unique(data$id)) #34 studies that can be used for slope calculation (kimura_geogr has 3 levels)
data<-unite(data,"popid",c("id","pop","genus","spec"),sep="-",remove=F)
length(unique(data$popid)) #201 populations (not 208 because lankinen)
...
levels(data$region)
#Japan: 83+10 from the "usa,japan" = 93
#Europe: 77
#US: 13+11 from the "usa,japan" = 24
#China: 7
...
18|paolucci     |1-4  |1                    |SCH removed
19|pegoraro     |NA   |NA                    |removed
20|pullin       |2    |2      |4
21|riihima      |2    |2      |4            |removed pops oulo 1+2, kuusamo3
22|ryan         |1,2,3|1      |4
23|shimizu      |3,4  |3                    |consider removing SPR
24|shintani     |LL.2 |LL.2                 |mod1 would have NAs, evidence from 8 further pops
25|shroyer      |NA   |NA                   |removed
26|so           |NA   |NA                   |removed
27|suwa         |1    |1      |2-4          |removed pops 14,15,16,18,21,23,31
28|tyukmaeva    |3,4  |3                    |removed pops pudasjarvi2, paltamo1, ivas1,lathi1,lathi3
29|ujiye        |NA    |NA      |NA         |removed
30|urbanski     |3    |3      |4              |1 and 2 without box constraints (bc fit is soo bad), 4 has NA for 1 pop
31|vaznunes     |3    |3      |4            |removed S1,S2, T1,T2
32|wang         |1,3  |1
33|yoshida      |3    |3
34|kimura_evol  |NA   |NA                   |removed
35|takeda       |1,3,4|1                    |consider removing
36|noda         |3    |3      |               |removed dl<9
...
#removed 9 studies (33 pops) and 27 further pops
#201-33-27 = 141 pops left
#in: 27 studies (though one study has 3 specs)
...
results$invse_b<-1/results$bse
plot(1:141,results$invvar_b, main = "influnece of data points on b",ylab="inverse variance [log]",log="y")
...
results$popid[results$invvar_b>med*10]


#maybe remove those populations (likely overfitted)?


#same with e (CDL)
results$invvar_e<-1/(results$ese^2)
results$invse_e<-1/results$ese
plot(1:141,results$invvar_e, main = "influnece of data points on e",ylab="inverse variance [log]",log="y")
...
results$popid[results$invvar_e>med*50]
...
results<-restore
results[(1/results$bse)>median(1/results$bse)*5,27]<-"e_only"
