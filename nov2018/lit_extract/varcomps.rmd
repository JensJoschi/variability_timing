...
calc_var <- function(row){
  ...
  y<-(d-c)/(1+exp(b*(x-e)))+c
  c( 
      (sum(y*(1-y))/1000), #variance within
      (sd(y)^2)            #variance between
      )
}
...
for (dataset in 1:number){
  ...
  for (i in (1:chains)){
    ...
    sav <- cbind(sav, temp, dat[,4]) #order: within, between, e, within ...[4 times]
  }
  sav<-sav[,-1]
  rm(list=outname)
...
scale <- 0.9
num <- 40
png("forest_b.png",width=480, height = 480*2, pointsize=12)
par(mfrow = c(1,2),mar = c(4,2,1,0))
plot(NA,xlim=c(8,20),ylim=c(1,num+2), xlab = "CDL", ylab ="",bty="n",yaxt="n" )
abline(h=1:num,col="lightgrey")
...
for (dataset in 1:num){
  ...
  res<-read.table(file = paste("res_", currpop, ".txt", sep = "" ))
   ...
   for ( i in 1:chains){
    dens<-density(res[,i*3])
    lines(x=dens$x,y = dataset+ scale * dens$y/max(dens$y),col=5-i)
   # points(x= dens$x[which.max(dens$y)],y = dataset + 0.5,col =i,bg = i, pch=22)
   }
  abline(h=dataset,col="lightgrey")
  segments (x0 = quantile(res[,3], 0.025), x1= quantile(res[,3], 0.975), y0 = dataset,y1 = dataset,col=4) #draw 95% credible interval for first chain
  points(x =median (res[,3]), y = dataset, cex=0.3, pch=22, bg=1)
  text(x=16, y = dataset+0.5, pos = 4, labels = unique(data$ID[data$popid == currpop]), cex = 0.8, col ="darkgrey") #author
  text(x=8,  y = dataset+0.5, pos = 4, labels = unique(data$pop[data$popid==currpop]), cex = 0.8, col = "darkgrey") #pop
}

##another copy to be plotted right next to it
## only changes to above: dataset in header of loop,  "dataset-num" whenever dataset is mentioned
plot(NA,xlim=c(8,20),ylim=c(1,num+2), xlab = "CDL", ylab ="",bty="n",yaxt="n" )
abline(h=1:num,col="lightgrey")

for (dataset in ((num+1):(2*num))){
  currpop <- unique(data$popid)[dataset]
  res<-read.table(file = paste("res_", currpop, ".txt", sep = "" ))

   for ( i in 1:chains){
    dens<-density(res[,i*3])
    lines(x=dens$x,y = dataset-num+ scale * dens$y/max(dens$y),col=5-i)
   # points(x= dens$x[which.max(dens$y)],y = dataset + 0.5,col =i,bg = i, pch=22)
   }
    abline(h=dataset-num,col="lightgrey")
  segments (x0 = quantile(res[,3], 0.025), x1= quantile(res[,3], 0.975), y0 = dataset-num,y1 = dataset-num,col=4) #draw 95% credible interval for first chain
  points(x =median (res[,3]), y = dataset-num, cex=0.3, pch=22, bg=1)
  text(x=16, y = dataset+0.5-num, pos = 4, labels = unique(data$ID[data$popid == currpop]), cex = 0.8, col ="darkgrey") #author
  text(x=8,  y = dataset+0.5-num, pos = 4, labels = unique(data$pop[data$popid==currpop]), cex = 0.8, col = "darkgrey") #pop
}
...
## same for variance within
```{r}
scale <- 0.9
num <- 40
png("forest_w.png",width=480, height = 480*2, pointsize=12)
par(mfrow = c(1,2),mar = c(4,2,1,0))
plot(NA,xlim=c(-0.025,0.275),ylim=c(1,num+2), xlab = "Variance within environments", ylab ="",bty="n",yaxt="n" )
abline(h=1:num,col="lightgrey")
...
for (dataset in 1:num){
  ...
  res<-read.table(file = paste("res_", currpop, ".txt", sep = "" ))
   ...
   for ( i in 1:chains){
    dens<-density(res[,i*3-2])
    lines(x=dens$x,y = dataset+ scale * dens$y/max(dens$y),col=5-i)
   # points(x= dens$x[which.max(dens$y)],y = dataset + 0.5,col =i,bg = i, pch=22)
   }
  abline(h=dataset,col="lightgrey")
  segments (x0 = quantile(res[,1], 0.025), x1= quantile(res[,1], 0.975), y0 = dataset,y1 = dataset,col=4) #draw 95% credible interval for first chain
  points(x =median (res[,1]), y = dataset, cex=0.3, pch=22, bg=1)
  text(x=0.2, y = dataset+0.5, pos = 4, labels = unique(data$ID[data$popid == currpop]), cex = 0.8, col ="darkgrey") #author
  text(x=-0.025,  y = dataset+0.5, pos = 4, labels = unique(data$pop[data$popid==currpop]), cex = 0.8, col = "darkgrey") #pop
}
...
for (dataset in ((num+1):(2*num))){
  ...
  res<-read.table(file = paste("res_", currpop, ".txt", sep = "" ))
   ...
   # points(x= dens$x[which.max(dens$y)],y = dataset + 0.5,col =i,bg = i, pch=22)
    ...
    abline(h=dataset-num,col="lightgrey")
  segments (x0 = quantile(res[,1], 0.025), x1= quantile(res[,1], 0.975), y0 = dataset-num,y1 = dataset-num,col=4) #draw 95% credible interval for first chain
  points(x =median (res[,1]), y = dataset-num, cex=0.3, pch=22, bg=1)
  text(x=0.2, y = dataset+0.5-num, pos = 4, labels = unique(data$ID[data$popid == currpop]), cex = 0.8, col ="darkgrey") #author
  text(x=-0.025,  y = dataset+0.5-num, pos = 4, labels = unique(data$pop[data$popid==currpop]), cex = 0.8, col = "darkgrey") #pop
}
...
for (dataset in 1:num){
  currpop <- unique(data$popid)[dataset]
  res<-read.table(file = paste("res_", currpop, ".txt", sep = "" ))
  estimates_e[dataset,]<-quantile(res[,3], c(0.025,0.5,0.975))
...
plot(r~jitter(a),pch=22, cex=1, col="grey",ylim=c(0,5)) # no decrease with sample size
plot(estimates_e$med~a)
r <- estimates_w$upper-estimates_w$lower
plot(r~jitter(a))
plot(estimates_w$med~a)
```
