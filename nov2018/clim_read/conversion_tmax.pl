...
#daily max temperatures. it ignores all other data flags that are inside that dataset. The format of the old datasets is fixed-width delimited, the new one will be delimited by "|".
...
my $readdir = "$newdir/01raw/ghcnd_all/ghcnd_all/ghcnd_all/";

print "\nmoved current working directory to $readdir\n";


my $writedir = "$newdir/02processing/001data_conversion";
my $writefile = ">>$writedir/converted_max.txt";
...
foreach $file (@files) {
	...
	while(my $line = <IN>){
	if ($line =~ m/TMAX/ ){ #the stations data contains many more information, e.g. precipitation data, which will not be needed. This line selects only information about maximunm daily temperature
		chomp $line;
		$i++;
		$rows[$i] = $line;
