...
#daily min temperatures. it ignores all other data flags that are inside that dataset. The format of the old datasets is fixed-width delimited, the new one will be delimited by "|".
...
my $writefile = ">>$writedir/converted_min.txt";
...
foreach $file (@files) {
	...
	while(my $line = <IN>){
	if ($line =~ m/TMIN/ ){ #the stations data contains many more information, e.g. precipitation data, which will not be needed. This line selects only information about minimum daily temperature
		chomp $line;
		$i++;
		$rows[$i] = $line;
