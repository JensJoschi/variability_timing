...
#nakagawas R²
(modnull$sigma2-varc_tp$sigma2)/modnull$sigma2  #  -0.047561935  0.001780985 -0.001627487
(sum(modnull$sigma2)-sum(varc_tp$sigma2))/sum(modnull$sigma2) #-0.00161734

#significance testing
anova(varc_tp.ml,modnull.ml) #LRT ratio =  0.3217 , p=0.5706
AIC(varc_tp.ml)-AIC(modnull.ml)#  1.678301
...
summary(cbh_dl)
#           estimate      se     zval    pval    ci.lb   ci.ub   
#intrcpt      0.1713  0.3096   0.5535  0.5799  -0.4354  0.7781   
#sd_winter   -0.0060  0.0282  -0.2117  0.8324  -0.0613  0.0494   

#R² = 0
anova(cbh_dl)#0.0448, p = 0.8324
#AIC diff: 0.1996261


svg("cbh_dl.svg", width = 10, pointsize=12)
par(mar = c(5,5,0,0)+0.1)
plot(res_mw ~ studies$sd_winter, pch=21, cex= 0.3+1.5*(1/vi)/(max(1/vi)) , col=1, bg=studies$col, main = "", ylab = "Residulas diapause timing", xlab = "Winter Unpredictability (day length)",bty="n", cex.axis = 1.5, cex.lab=1.5,xlim=c(0,26), ylim=c(-3,4)) #cex again set so that max ptsize = 2
#legend("bottomright",legend=names(n), col=rep(1,8), pt.bg=unique(studies$col), pch=21, bty="n", ncol=1,cex=1.5)
...
summary(cbh_t)
#         estimate      se    zval    pval    ci.lb   ci.ub   
#intrcpt    0.0703  0.2223  0.3163  0.7518  -0.3653  0.5059   
#beta       0.1183  0.5705  0.2073  0.8358  -0.9999  1.2364   
...
par(mar = c(5,5,0,0)+0.1)
plot(res_mw ~ studies$beta, pch=21, cex= 0.3+1.5*(1/vi)/(max(1/vi)) , col=1, bg=studies$col, main = "", ylab = "Residulas diapause timing", xlab = "Winter unpredictability (temperature)",bty="n", cex.axis = 1.5, cex.lab=1.5,xlim=c(-1,1.5), ylim=c(-3,4)) #cex again set so that max ptsize = 2
#legend("bottomright",legend=names(n), col=rep(1,8), pt.bg=unique(studies$col), pch=21, bty="n", ncol=1,cex=1.5)
abline(cbh_t,lwd=2)
dev.off()
...
par(mar = c(5,5,0,0)+0.1)
plot(res_mw ~ res_varc, pch=21, cex= 0.3+1.5*(1/vi)/(max(1/vi)) , col=1, bg=studies$col, main = "", ylab = "Residulas diapause timing", xlab = "Residuals variance composition",bty="n", cex.axis = 1.5, cex.lab=1.5,xlim=c(-3,3.5), ylim=c(-3,3)) #cex again set so that max ptsize = 2
#legend("bottomright",legend=names(n), col=rep(1,8), pt.bg=unique(studies$col), pch=21, bty="n", ncol=1,cex=1.5)
...
for ( i in 1:nrow(studies)){
  ...
  studies <- studies[!is.na(studies$meanwinter),]
studies$mw<-NA
studies$mwl<-NA
studies$mwh<-NA
studies$meanwinter <- studies$meanwinter + 182 #was counted in days since midsummer
