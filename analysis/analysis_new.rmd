...
metaf<-rma.mv(yi = med_e ~ degN, V=1, random = ~1|order/genus/spec/ID,data = studies) 
#this model has very little vraiance explained by ID, because spec and genus absorb most of it
...
```{r degN_models}
#unweighted models
metaf<-rma.mv(yi = med_e ~ degN, V=1, random = ~1|order/spec,data = studies) 
glmmtmb <- glmmTMB( med_e ~ degN + (1|order/spec), data=studies ) #nearly same


#weighted
metaf<-rma.mv(yi = med_e ~ degN, V=vi, random = ~1|order/spec,data = studies) 
w<-1/vi 
w<-w/sum(w) #weight has a sum of 1 now; this affects glmmtmb, see "weights" in ?glmmTMB
glmmtmb <- glmmTMB( med_e ~ degN + (1|order/spec), data=studies, weights=w, REML = F) 


#null models and ML models (weighted)
metaf.ml<-rma.mv(yi = med_e ~ degN, V=vi, random = ~1|order/spec,data = studies, method = "ML") 
metanull<-rma.mv(yi = med_e ~ 1, V=vi, random = ~1|order/spec,data = studies,method = "ML") 
glmmtmb.ml <- glmmTMB( med_e ~ degN + (1|order/spec), data=studies, weights=w, REML = F) 
tmbnull <- glmmTMB( med_e ~ 1 + (1|order/spec), data=studies, weights=w) 
