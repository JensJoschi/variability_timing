...
#         estimate      se     zval    pval   ci.lb   ci.ub     
#intrcpt  316.9942  8.6379   36.6980  <.0001  300.0642  333.9242  ***
#degN      -1.8763  0.1426  -13.1575  <.0001   -2.1558   -1.5968  ***

#mwN2: 314.9805  8.6047;   -1.8115  0.1456
#mwN3: 316.7157  8.6865; -1.8671  0.1440 
#mw_N_year: 317.0758  8.6817;   -1.8756  0.1438
plot(resid(mw_N))
...
#nakagawas R²
(metanull$sigma2-mw_N$sigma2)/metanull$sigma2 #   -0.08697811  0.05505461  0.35055292
(sum(metanull$sigma2)-sum(mw_N$sigma2))/sum(metanull$sigma2) #0.1384581
#significance testing
anova(metaf.ml,metanull.ml) #LRT ratio =  141.7762, p<.0001
...
#            estimate       se     zval    pval    ci.lb     ci.ub     
#intrcpt      83.9671  12.0830   6.9492  <.0001  60.2850  107.6493  ***
#meanwinter    0.5079   0.0339  14.9977  <.0001   0.4415    0.5743  ***

#mw2: 86.7158  12.2406 ;  0.5018   0.0347
#mw3:  84.8622  12.1509; 0.5057   0.0340
#year:  84.3096  12.1586 ; 0.5078   0.0340

plot(resid(mw))

#nakagawas R²
(mwnull$sigma2-mw$sigma2)/mwnull$sigma2  #-0.08690513  0.18344098  0.41092969
(sum(mwnull$sigma2)-sum(mw$sigma2))/sum(mwnull$sigma2) #   0.2131471

#significance testing
anova(mw.ml,mwnull.ml) #LRT ratio =  175.6902, p<.0001
AIC(mw.ml)-AIC(mwnull.ml)#    -173.6902
...
summary(mod)
#Model Results:
#intrcpt      1.7696  0.2817   6.2823  <.0001   1.2175   2.3217  ***
#sd_winter   -0.1202  0.0179  -6.7156  <.0001  -0.1553  -0.0851  ***

transf.ilogit(-0.1202)#  0.4699861

#mod2: 0.4693634
#mod3: 0.4689151
#year: 0.4703598
#nakagawas R²
(modnull$sigma2-mod$sigma2)/modnull$sigma2   # -4.355522e+06  3.160831e-01  9.638133e-02
(sum(modnull$sigma2)-sum(mod$sigma2))/sum(modnull$sigma2)  # 0.1266816

#significance testing
anova(mod.ml,modnull.ml) #LRT ratio =  41.3545, p<.0001
AIC(mod.ml)-AIC(modnull.ml)#  -39.35448
...
summary(cbh_dl)
#           estimate      se     zval    pval    ci.lb   ci.ub   
#intrcpt      0.0254  0.3264  0.0779  0.9379  -0.6142  0.6651   
#sd_winter    0.0209  0.0310  0.6735  0.5006  -0.0398  0.0815 

#R² = 0
anova(cbh_dl)#  0.4536p =   p-val = 0.5006
#AIC diff:  -1.977
