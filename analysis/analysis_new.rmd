...
for ( i in 1:nrow(studies)){
  ...
  studies$sd_p[i]<-sd(sub$p) #quality control. the predictability of nearby stations should be similar to each other
...
```{r varcomp}
x<-studies # a backup
studies<-studies[!is.na(studies$p),] #3 pops removed
studies[studies$p>4,]#sth is strange about obernai - inachis io
hist(studies$sd_p,breaks=100)#this is the only point in the dataset with high standard deviation in predictability among close stations
#strassbourg( close to obernai) has a predictability of 10, all other stations in +-5 km a predictability between 1 and 2. 
studies <- studies[studies$p<4,]
...
mod <- rma.mv(med_r ~ p,vi, random = ~ 1 | order/spec/popid, data=studies)
plot(rstandard(mod)$z, ylim=c(-3,3), pch=19) #not very nice
studies$yti <- transf.logit(studies$med_r)
studies$vti <- studies$r / (studies$med_r * (1 - studies$med_r))^2
mod <- rma.mv(yti ~ p, vti, random = ~ 1 | order/spec/popid, data=studies)

mod
plot(rstandard(mod)$z, pch=19)
plot(studies$p, studies$yti, pch=19)
