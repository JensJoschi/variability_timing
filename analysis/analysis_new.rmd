...
studies$col<-as.numeric(studies$order) #numbers from 1:9
studies$col<-viridis(9)[studies$col]
...
#intrcpt    6.9807  0.3709  18.8231  <.0001  6.2538  7.7076  ***
#degN       0.1619  0.0063  25.6976  <.0001  0.1495  0.1742  ***
#48.57 min per 5 deg N
...
#nakagawas R²
(metanull$sigma2-metaf$sigma2)/metanull$sigma2 #-11.2652101   0.6918200   0.6192251

(sum(metanull$sigma2)-sum(metaf$sigma2))/sum(metanull$sigma2) #0.58165

#significance testing
anova(metaf.ml,metanull.ml) #LRT ratio = 400.3325, p<.0001
AIC(metaf.ml)-AIC(metanull.ml)#     -398.3325
...
#for 22 studies the calculation of diapause reaction norms was not reasonable( studies conducted at unreasonable day lengths), 13 do not have any climate station nearby. for 2 of those (beach 14°N; musolin okinawa), both criteria apply, so there are 33 reaction norms to remove
...
#         estimate      se     zval    pval   ci.lb   ci.ub     
#intrcpt  317.2594  8.3331   38.0722  <.0001  300.9268  333.5919  ***
#degN      -1.8755  0.1426  -13.1548  <.0001   -2.1549   -1.5960  ***

#mwN2: 315.4311  8.3472;  -1.8135  0.1458
#mwN3: 316.9558  8.3864;-1.8652  0.1441
#mw_N_year: 316.9558  8.3864;   -1.8652  0.1441 
plot(resid(mw_N))
...
#nakagawas R²
(metanull$sigma2-mw_N$sigma2)/metanull$sigma2 #   -0.04609830  0.05612879  0.35045230
(sum(metanull$sigma2)-sum(mw_N$sigma2))/sum(metanull$sigma2) #0.1501087
#significance testing
anova(metaf.ml,metanull.ml) #LRT ratio =  141.8189, p<.0001
AIC(metaf.ml)-AIC(metanull.ml)#   -139.8189
...
which.min(x<sum(1/vi)*0.9) # 223
223/425 #52% of the points account for 90% of the total weight
```
...
#            estimate       se     zval    pval    ci.lb     ci.ub     
#intrcpt       84.4230  11.9306   7.0762  <.0001  61.0394  107.8066  ***
#meanwinter     0.5081   0.0339  14.9963  <.0001   0.4417    0.5745  ***

#mw2: 87.0599  12.1009 ; 0.5020   0.0347 
#mw3: 85.3778  11.9987; 0.5056   0.0340
#year: 84.7304  11.9964  ;0.5075   0.0340

plot(resid(mw))

#nakagawas R²
(mwnull$sigma2-mw$sigma2)/mwnull$sigma2  #-0.06021944  0.18414530  0.41126139
(sum(mwnull$sigma2)-sum(mw$sigma2))/sum(mwnull$sigma2) #  0.2223478

#significance testing
anova(mw.ml,mwnull.ml) #LRT ratio =  175.6366, p<.0001
AIC(mw.ml)-AIC(mwnull.ml)#    -173.6366
...
summary(mod)
#Model Results:
#intrcpt      1.7774  0.2805   6.3361  <.0001   1.2276   2.3271  ***
#sd_winter   -0.1197  0.0179  -6.7073  <.0001  -0.1547  -0.0847  ***
transf.ilogit(-0.1197)# 0.4701107

#mod2: 0.4693385
#mod3: 0.4688902
#year: 0.4699114
#nakagawas R²
(modnull$sigma2-mod$sigma2)/modnull$sigma2   #  -4.091876e+06  3.268443e-01  9.549636e-02
(sum(modnull$sigma2)-sum(mod$sigma2))/sum(modnull$sigma2)  #0.1232669

#significance testing
anova(mod.ml,modnull.ml) #LRT ratio =  41.1793, p<.0001
AIC(mod.ml)-AIC(modnull.ml)#  -39.17931
...
summary(cbh_dl)
#           estimate      se     zval    pval    ci.lb   ci.ub   
#intrcpt      0.0059  0.3264  0.0182  0.9855  -0.6337  0.6456   
#sd_winter    0.0211  0.0310  0.6806  0.4961  -0.0396  0.0818   

#R² = 0
anova(cbh_dl)#  0.4633 p =   p-val = 0.4961
#AIC diff:  -1.987
