...
#         estimate      se     zval    pval   ci.lb   ci.ub     
#intrcpt  318.2257  8.4909   37.4785  <.0001  301.5839  334.8675  ***
#degN      -1.8995  0.1457  -13.0399  <.0001   -2.1850   -1.6140  ***
...
#nakagawas R²
(metanull$sigma2-mw_N$sigma2)/metanull$sigma2 #  -0.15030588 -0.00888036  0.35462072
(sum(metanull$sigma2)-sum(mw_N$sigma2))/sum(metanull$sigma2) #0.1096057
#significance testing
anova(metaf.ml,metanull.ml) #LRT ratio =  136.9356, p<.0001
AIC(metaf.ml)-AIC(metanull.ml)#  -136.6441
...
#            estimate       se     zval    pval    ci.lb     ci.ub     
#intrcpt      83.2647  12.1857   6.8330  <.0001  59.3813  107.1482  ***
#meanwinter    0.5116   0.0347  14.7284  <.0001   0.4435    0.5796  ***

plot(resid(mw))

#nakagawas R²
(mwnull$sigma2-mw$sigma2)/mwnull$sigma2  # -0.1370383  0.1484955  0.4085796
(sum(mwnull$sigma2)-sum(mw$sigma2))/sum(mwnull$sigma2) # 0.1977801

#significance testing
anova(mw.ml,mwnull.ml) #LRT ratio = 169.9964, p<.0001
AIC(mw.ml)-AIC(mwnull.ml)#     -167.9964
...
summary(mod)
#Model Results:
#intrcpt      1.6791  0.2799   5.9991  <.0001   1.1305   2.2277  ***
#sd_winter   -0.1103  0.0176  -6.2705  <.0001  -0.1448  -0.0758  ***
transf.ilogit(-0.1103)#  0.4724529


#nakagawas R²
(modnull$sigma2-mod$sigma2)/modnull$sigma2   # -7.481264e+06  3.141777e-01  8.415775e-02
(sum(modnull$sigma2)-sum(mod$sigma2))/sum(modnull$sigma2)  #0.1126181

#significance testing
anova(mod.ml,modnull.ml) #LRT ratio =  36.1787, p<.0001
AIC(mod.ml)-AIC(modnull.ml)# -34.17869
...
summary(cbh_dl)
#           estimate      se     zval    pval    ci.lb   ci.ub   
#intrcpt      0.0942  0.3204  0.2939  0.7689  -0.5338  0.7222   
#sd_winter    0.0144  0.0298  0.4825  0.6294  -0.0441  0.0728  

#R² = 0
anova(cbh_dl)# 0.2328, p =  0.6294
#AIC diff:  -1.723492
