...
reload the temp dataset and remove dates like 31 feb
```{r reload}
rm(station)
rm(converted)
rm(slist)
...
```{r testing}
beta<-rep(NA,n_distinct(station$ID))
obs<-rep(sin((1:366)/366 * 2* pi),30)
t<-data.frame(rep(NA,366*30))
for( i in 1:366){
  t[,i]<-(rep(sin((1:366)/i * 2* pi),30))
}
t<-t[,60:366]
freqvec<-seq(1,0,length.out=307)
new<-data.frame(rep(NA,nrow(t)))
for (i in 1:ncol(t)){
new[,i]<-t[,i]*freqvec[i]
}
plot(rowSums(new,na.rm=T),type="l")

#obs<-obs+rnorm(length(obs),0.2)
  #obs<-data.frame(obs)
  obs<-rowSums(new,na.rm=T)
  ys           <- nrow(obs)/366
