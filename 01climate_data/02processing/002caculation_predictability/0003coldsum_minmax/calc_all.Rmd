...
rm(converted)
rm(slist)
converted<-read_delim("temp.txt",delim = " ",col_names = c("ID","year",1:372))
#739 fails, whenever a whole month is missing

#converted<-fread("temp.txt",sep =" ",col.names=c("ID","year",1:372),nrows=1231)
#length(station$`08Feb_29`) #not many, due to leapyears
#x<-NA
#for(t in 1:ncol(station)){x[t]<-n_distinct(station[,t])}
#station<-station[-which(x==1)]  #dates like 30 feb are erased #this causes some trouble, stations may which happen to have no temp recording on 15 august will have one column less
```
...
mean_winter<-rep(NA,length(unique(converted$ID)))
sd_winter<- rep(NA,length(unique(converted$ID)))
nyears<-rep(NA,length(unique(converted$ID)))

stations<-unique(converted$ID)
for (i in 1:length(stations)){ #this loop goes through each of the 1 mil station
