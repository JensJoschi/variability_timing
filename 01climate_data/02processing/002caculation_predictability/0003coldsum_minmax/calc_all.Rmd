...
station<-select(converted,which(x>0))
```
...
ndays_300<-rep(NA,length(unique(converted$ID)))
ndays_350<-rep(NA,length(unique(converted$ID)))
ndays_365<-rep(NA,length(unique(converted$ID)))
...
reslist<-data.frame(unique(station$ID),mean_winter,sd_winter,nyears,ndays_300,ndays_350,ndays_365)

names(reslist)<-c("ID","meanwinter","sd_winter","nyears","ndays_300","ndays_350","ndays_365")
