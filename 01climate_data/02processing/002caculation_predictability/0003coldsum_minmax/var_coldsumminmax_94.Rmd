...
# issues:
careful: threhsold is saved in the workspace, loading workspace replaces threshold and t2. 
...
### Overview  
This script is a copy of var_coldsumminmax.rmd, but uses parameters that achieve a winter onset of 94 days earlier than in the former version. The reason is that a winter onset of -94 days fits best to the CDL estimates. The winter onset used so far resulted in a mean winter onset of december 1st, which is the definition of meteorological winter. But it seems that this is too late for insects.
...
### Script  
Has currently some adaptations for testing (only mean winter onset)
...
write.table(reslist,"results_94.txt",append=TRUE,quote=FALSE,row.names=FALSE,col.names=FALSE)
} #next chunk
```
