...
# The function tries up to three times per climate station, with different starting parameters.
 ...
 freqs<-n[,2]
xvector<-1:length(freqs)
ms<-rep(NA,366)
for ( k in 1:366){
  ms[k]<-mean(freqs[xvector%%366==k],na.rm=T)
}

freq2<-freqs
for ( k in 1:366){
  freq2[xvector%%366==k]<-freq2[xvector%%366==k]-ms[k]
}

freq2<-na.interpolation(freq2)
spec<-spectrum(freq2[!is.na(freq2)],plot=F,method="ar")
#missing: linear interpolation to deal with NAs
spec$spec<-spec$spec[spec$freq>0]
spec$freq<-spec$freq[spec$freq>0]
beta[i]<- exp(coef(lm(log10(spec$spec)~log10(spec$freq)))[2])
