...
get_nls <- function (vals,s_A=400,s_phi=pi/2,s_c=200){#s_... are starting values for nls function
  if (length(vals)==NA || length(vals)<1000){return (NA)} else{ #when there is insufficient data, the function daily_t will return "NA", so vals gets length 1. Otherwise vals will have length of at least 2*370(many of which may be NA)
  x<-1:length(vals)
...
for (i in 1:length(unique(station$ID))){
  ...
  ##calculate winter arrival: make a binomial matrix with all Temps<threshold = 1, all others =0(including NA)
  bino<-s
  ...
  #calculate correlation with x days ago - the mean correölation was normally distributed around 0 and hence discontinued
...
reslist<-data.frame(unique(station$ID),mean_winter,sd_winter,nyears,c)

names(reslist)<-c("ID","meanwinter","sd winter","nyears","cor")
