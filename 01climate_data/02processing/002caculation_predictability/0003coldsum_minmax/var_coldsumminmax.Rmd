...
r2$cor<-r2$cor+1
r2<-r2[!is.na(r2$cor),]
r2$`sd winter`[r2$`sd winter`>50]<-50
hist(r2$`sd winter`,breaks=100)
r2<-r2[r2$V2<60,]
plot(r2$V2~r2$V3,bg=rgb(r2$cor,r2$cor,0,maxColorValue = max(r2$cor)),pch=22,col=NA,cex=0.4,main="predictability")#the darker, the higher the neg. correlation
plot(r2$V2~r2$V3,bg=rgb(r2$`sd winter`,r2$`sd winter`,0,maxColorValue = max(r2$`sd winter`)),pch=22,col=NA,cex=1,main="sd winter")
...
r2$V4[r2$V4<(-990)]<-NA
r2$V4[r2$V4<0]<-0 #only 14 values, between 0 and -22m
M<-lm(r2$cor~r2$V2+r2$V3+sqrt(r2$V4))
summary(M)#r2 0.27
plot(M)

#filtering for usa only
am<-r2[r2$V3<(-50),]
plot(am$V2~am$V3,bg=rgb(am$cor,am$cor,0,maxColorValue = max(am$cor)),pch=22,col=NA,cex=0.4,main = "predictability")#
```
...
names(da)=c("r2$V2","r2$V3","sqrt(r2$V4)")
predictions<-predict(M,newdata=da)
da$preds<-coef(M)[1] + coef(M)[2] * da[,1] + coef(M)[3]*da[,2] + coef(M)[4] * da[,3]
plot(da[,1]~da[,2],bg=rgb(da$preds,da$preds,0,maxColorValue = max(da$preds)),pch=22,cex=0.1,col=NA)

#us only
us<-da[da$`r2$V3`<(-50),]
plot(us[,1]~us[,2],bg=rgb(us$preds,us$preds,0,maxColorValue = max(us$preds)),pch=22,cex=0.4,col=NA)
```
