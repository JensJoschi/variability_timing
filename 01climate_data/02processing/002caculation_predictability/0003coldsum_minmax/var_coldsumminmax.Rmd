...
Further processing uses smaller chunks of data at a time (30 chunks of 1k climate stations). These are subsequently read in and transformed to differen format (so that 1 row has temperature of a whole year). Then I calculate winter onset as xth day that temperature falls below y°C. For each station I calculate mean winter onset (weighted by available data per year), standard deviation in winter onset, and winter predictability: The last 31 days before winter onset of each year are aggregated and pearsons R is calculated. The climate at one station is considered predictable (high R) if 1) temperature declines more or less linearly, 2) the slope is consistent across years, and 3) there is little day-to-day variation. Finally, all data of one station is put into a single vector, and I apply a non-linear least square regression: The data is expected to follow a sine-curve with a period of 365 days, and I record for each station amplitude (difference between summer and winter), phase angle (day of peak summer) and intercept (mean temperature). The results (mean winter onset, sd, correlation, nls-parameters) are saved in a text file. All of this is done on all chunks of data, and the results are appended in a single file.  

In the end, the results are read in again, and merged with climate station metadata (needs to connects with the NOAA server to get a list of available climate stations together with their latitudes and longitudes. This is the only time a connection to the server is required).mean, predictability and standard deviation of winter onset are correlated with latitude, longitude and altitude. The model coefficients are used to also infer predictability and variability for the space between stations, using a topographic map of the world
...
* make subset with 1 station and
  ...
  ** calculate correlation of winter onset and temperature 1,2,3 weeks before winter onset
  ** calculate predictability: pearsons R along last 30 days before winter onset
...
station<-
  ...
  mutate (day=str_pad(day,2,pad="0")) %>%  #this makes leading zeros where they aer needed
...
for (i in 1:length(unique(station$ID))){
 ...
 # make a binomial matrix with all Temps<threshold = 1, all others =0(including NA)
  ...
  ##calculate mean and sd in witner arrival (both weighted)####
  ndays<-ndays[used]
  ...
  #calculate correlation with x days ago ####
  #- the mean correölation was normally distributed around 0 and hence discontinued
...
#  w_14<-s[m3]
 ...
 nyears[i] <- length(w_on)
  #idea: transpose and use gather to make 1 long vector ("day1:day 10000" only that they are not called like that)
        #cut at (winter onset - 30) and winter onset
        #clean incomplete data
        #make correlation aggregating data from all years
  ...
  n2<-group_by(n,key)
  test<-as.data.frame(summarise(n2,n())[,2])
  full_seq<-NULL
  for (k in 1:nrow(test)){
   full_seq<-c(full_seq,(1:test$`n()`[k]))
  }
  n2$xvec2<-full_seq
  ...
  #correlate
 c[i]<- cor(n$value,n$xvec2,use="na.or.complete") #uses xvec 2, this aligns the data from all years on the x-axis
...
reslist<-data.frame(unique(station$ID),mean_winter,sd_winter,nyears,c,A,phi,int)

names(reslist)<-c("ID","meanwinter","sd winter","nyears","cor","amplitude","phase angle", "intercept")
...
names(r2)<-c("ID","lat","lon","alt","no_idea","name","meanwinter","sd winter", "nyears","cor","amplitude","phase angle", "intercept")
...
hist(r2$`sd winter`,breaks=100)
r2$capped_sd<-r2$`sd winter`
r2$capped_sd[r2$capped_sd>50]<-50

hist(r2$nyears,breaks=50)

hist(r2$cor,breaks=100)

hist(r2$amplitude,breaks=100)
r2$capped_A<-r2$amplitude
r2$capped_A[r2$capped_A>200]<-200

hist(r2$`phase angle`,breaks=100)
hist(r2$intercept)

p<-r2
p<-p[!is.na(p$intercept),]
p$intercept<-p$intercept + abs(min(p$intercept))
plot(p$lat~p$lon,bg=rgb(p$intercept,p$intercept,0,maxColorValue = max(p$intercept)),pch=22,cex=0.3,col=NA,main="annual mean temp")
cor(r2$intercept,r2$meanwinter,use="na.or.complete")


p<-r2
p<-p[!is.na(p$amplitude),]
plot(p$lat~p$lon,bg=rgb(p$capped_A,p$capped_A,0,maxColorValue = max(p$capped_A)),pch=22,cex=0.3,col=NA,main="Amplitude")


p<-r2
p<-p[!is.na(p$`phase angle`),]
p$p2<-sin(p$`phase angle`)+1
plot(p$lat~p$lon,bg=rgb(p$p2,p$p2,0,maxColorValue = max(p$p2)),pch=22,cex=0.3,col=NA,main="phase angle")


p<-r2
p<-p[!is.na(p$meanwinter),]
plot(p$lat~p$lon,bg = rgb(p$meanwinter,p$meanwinter,0,maxColorValue = max(p$meanwinter)),cex=0.3,pch=22,col=NA, main ="mean winter onset")


p<-r2
p<-p[!is.na(p$capped_sd),]
p<-p[p$nyears>8,]
#plot(p$lat~p$lon,bg = rgb(p$capped_sd,p$capped_sd,0,maxColorValue = max(p$capped_sd)),cex=0.3,pch=22,col=NA, main ="sd winter onset, capped at 50")
p$capped_sd[p$capped_sd>20]<-20
plot(p$lat~p$lon,bg = rgb(p$capped_sd,p$capped_sd,0,maxColorValue = max(p$capped_sd)),cex=0.3,pch=22,col=NA, main ="sd winter onset, capped at 20")
plot(r2$`sd winter`~r2$amplitude,cex=0.3,pch=22,col=NA,bg=rgb(0,0,0,alpha=0.2,maxColorValue = 1))
summary(lm(r2$capped_sd~r2$lat+r2$amplitude+r2$alt))

p<-r2
p<-p[!is.na(p$cor),]
p$cor<-p$cor+abs(min(p$cor))
plot(p$lat~p$lon,bg = rgb(p$cor,p$cor,0,maxColorValue = max(p$cor)),cex=0.3,pch=22,col=NA, main ="predictability",sub="dark = high correlation")
summary(lm(r2$cor~r2$lat+r2$lon+r2$sq_alt))
r<-resid(lm(r2$cor~r2$lat+r2$lon+r2$sq_alt))
r[r>0.5]<-0.5
r<--r
r<-r+abs(min(r))
...
am<-r2[r2$lon<(-50),]
am<-am[!is.na(am$cor),]
am$cor<-am$cor+abs(min(am$cor))
plot(am$lat~am$lon,bg=rgb(am$cor,am$cor,0,maxColorValue = max(am$cor)),pch=22,col=NA,cex=0.3,main = "predictability")#
...
slopes<-separate(slopes,1,into=c("study","pop"),remove=FALSE,sep="-")
slopes$study[slopes$study=="6b"]<-6
plot(slopes$preds,slopes$b2,main = "environmental predictability vs diapause variability",xlab = "environment", ylab = "physiology",pch=22,bg=slopes$study)
