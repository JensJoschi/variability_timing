...
The results of the 4 runs are very similar, and the sd and CV plots are very dark. this is because the standard deviation is rather long-tailed, caused by the northernmost stations. Cropping it a bit to get a better view:
...
```{r calc_sdweighed}
weighmean<-NA
for (i in 1:nrow(final_10_50[[1]])){
  w_on<-final_10_50[[1]][i,4:(ncol(final_10_50[[1]])-3)]
  weights<-final_10_50[[2]][i,4:(ncol(final_10_50[[2]])-3)]
  w<-!is.na(w_on)
  weights<-weights[w]
  weights<-weights/372
  w_on <- w_on[w]
  mean_w_on<-weighted.mean(w_on,weights)
  #formula variance for weighed sample, using frequency weighed unbiased estimateor (i.e. bessel's correction)
  upper <- sum(weights*(w_on-mean_w_on)^2)
  lower <- sum(weights)-1
  if (lower >0){final_10_50[[1]]$sd[i] <- sqrt(upper/lower)} else {final_10_50[[1]]$sd[i]<-NA}
  weighmean[i]<-mean_w_on
}

hist(final_10_50[[1]]$sd,breaks=100)
capped<-final_10_50[[1]]
capped$sd[capped$sd>50]<-50
to_plot<-capped[is.na(capped$sd)==FALSE,] #copied and adapted from means plot
plot(to_plot[,2]~to_plot[,3],bg=rgb(0,to_plot$sd,0,maxColorValue=50),col=NA,main="Winter variability, weighed",sub= "sd =frequency weighed unbiased estiator",pch=22,cex=0.8)
points(x=(-170):(-121),y= rep(25,50), col = rgb (0, 0:50,0,maxColorValue = 50),pch=22,cex=0.8)
text(-170,22,"sd=0")
text(-120,22,"sd = 50")

#alternative
#mean<-weighted_means<-function(resm,nm)
#to_plot$c<-to_plot$sd/to_plot$mean
#to_plot$c[to_plot$c>0.5]<-0.5
#hist(to_plot$c,breaks=100)
#plot(to_plot[,2]~to_plot[,3],bg=rgb(0,to_plot$c,0,maxColorValue=max(to_plot$c)),col=NA,main="Winter variability, sd/n",pch=22,cex=0.8)
#points(x=(-170):(-121),y= rep(25,50), col = rgb (0, seq(0,max(to_plot$c),length.out=50),0,maxColorValue = max(to_plot$c)),pch=22,cex=0.6)
#text(-170,22,"sd/N = 1")
#t#ext(-120,22,"sd/N = 3")
```

###plotting mean winter onset and non-corrected winter variability
