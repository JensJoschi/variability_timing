...
te2<-test[,4:ncol(test)]
te<-final_10_50[[2]][,4:(ncol(final_10_50[[2]]))]
t<-!is.na(te)
hist(te[t][te[t]>100],breaks=100, xlab="Days",ylab ="Frequency",main ="Frequency distribution: Number of days/year")
length(te[t][te[t]>300])/length(te[t])
length(te[t][te[t]<100])/length(te[t])
```
```{r}
for (i in 1:nrow(final_10_50[[1]])){
  ...
  lower <- sum(weights)-1
  final_10_50[[1]]$sd[i] <- sqrt(upper/lower)
}
```
